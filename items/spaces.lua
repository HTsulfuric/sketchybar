...
for i, workspace in ipairs(workspaces) do
	...
	space:subscribe("mouse.clicked", function(env)
		...
		if env.BUTTON == "other" then
			...
			sbar.exec("aerospace workspace " .. SID) -- SIDはworkspace名になる
		end
	end)
	...
	for i, workspace in ipairs(workspaces) do
		sbar.exec(
			...
			function(apps) -- workspace名を指定
				...
				sbar.animate("tanh", 10, function()
					spaces[workspace]:set({ -- spacesテーブルのキーをworkspace名に変更
						label = icon_line,
					})
				end)
			end
